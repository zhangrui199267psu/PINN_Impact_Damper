...
class PIPNNs:
    ...
    def __init__(self, lb, ub, t0, t, x0_total, xt0_total, y0, yt0, M, K, D, n_dof, phi, phi1, phi2,
                 layers, hyp_ini_weight_loss, hyp_ini_para, optimizer_LB):
        ...
        self.loss = self.beta_icx * self.loss_icx + self.beta_fx * self.loss_fx+ self.beta_f * self.loss_f
    ...
    def net_f(self, t):
        ...
        y = self.yt0 * t + self.y0
        
        f = tf.math.abs(x[:, 3:4]-y)-self.D
        
        return fx, f
        
    def callback(self, loss, loss_icx, loss_fx, loss_f, lambda_1):
        ...
        print('Loss: %e, Loss_icx: %.5e, Loss_fx: %.5e, Loss_f: %.5e, l1: %.5f'
              % (loss, loss_icx, loss_fx, loss_f, lambda_1))

    def train(self, nIter, optimizer_LB):
        ...
        for it in range(nIter):
            ...
            if it % 10 == 0:
                ...
                print('It: %d, Loss: %.3e, Loss_icx: %.3e,  Loss_fx: %.3e, Loss_f: %.3e, Lambda_1: %.3f, Time: %.2f' %
                    (it, loss_value, loss_icx_value, loss_fx_value, loss_f_value, lambda_1_value,  elapsed))
                start_time = time.time()
