...
-------------------------------------
Standalone TensorFlow 2 PINN for an n-DOF impact-damper chain.

This file is self-contained (does not import pinn_ndof_chain_tf2.py) and keeps
the same training/prediction utilities, while using a no-external-force ODE
residual for the free-free initial-velocity scenario:

    M x_tt + C x_t + K x = 0

Use `build_free_free_chain_matrices(...)` and `make_left_velocity_ic(...)`
for the free-free 20-DOF launch case.

Author: Rui Zhang (base), adapted by Codex
"""
...
# ---------------------------------------------------------------------------
# Free-free helpers for no-force initial-velocity studies
# ---------------------------------------------------------------------------
