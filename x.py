...
# ---------------------------------------------------------------------------
# Quick-start example  (n = 10 DOFs, one impactor each)
# ---------------------------------------------------------------------------

if __name__ == '__main__':

    n = 10          # number of DOFs  (= number of impactors)
    num = 64        # hidden neurons per layer

    # ── System matrices ────────────────────────────────────────────────
    m_x = 1.0       # primary mass (same for all DOFs)
    m_y = 0.1       # impactor mass (same for all DOFs)
    k   = 100.0     # spring stiffness (neighbour coupling)
    c   = 0.5       # damping coefficient (neighbour coupling)
    D   = 1.0       # impact gap (same for all DOFs)
    r   = 0.7       # coefficient of restitution

    M = np.diag(m_x * np.ones(n))

    K = np.zeros((n, n))
    for i in range(n):
        K[i, i] = 2 * k if 0 < i < n - 1 else k
        if i > 0:     K[i, i-1] = -k
        if i < n - 1: K[i, i+1] = -k

    C = (c / k) * K   # proportional damping

    # ── Forcing parameters ─────────────────────────────────────────────
    phi1 = 1.0      # amplitude
    phi2 = 1.0      # frequency  →  F = phi1 * sin(phi2 * pi * (t + phi))

    # ── PINN hyper-parameters ──────────────────────────────────────────
    layers               = [1, num, num, n]
    hyp_ini_weight_loss  = np.array([100.0, 1.0])
    ...
    x0_total   = np.zeros((1, n))    # primary DOFs at rest
    xt0_total  = np.zeros((1, n))

    y0   = np.zeros(n)               # impactors start at x_i + D (top wall)
    yt0  = np.zeros(n)               # impactors at rest

    phi0 = np.array([[0.0]])         # accumulated phase at segment start

    t01  = np.linspace(0, T01[0], num01).reshape(-1, 1)

    # Velocity-update matrix  A^{-1} B  (momentum + restitution)
    mx, my = m_x, m_y
    A_inv_B = np.array([
        [(mx - r * my) / (mx + my),  my * (1 + r) / (mx + my)],
        [mx * (1 + r)  / (mx + my), (my - r * mx) / (mx + my)],
    ])

    print("Building model for segment 1  (n_dof=%d)..." % n)
    model01 = PIPNNs(
        lb, ub,
        t0, t01,
        x0_total, xt0_total,
    ...
    print("Training segment 1...")
    model01.train(nIter=nIter01, optimizer_LB=optimizer_LB_value)

    print("Finding impact times (Phase 2)...")
    t_impacts01, hit01 = find_impact_times(
        model01, y0, yt0, D, float(T01[0])
    )

    first_hit_dof = int(np.argmin(t_impacts01))
    t_seg01       = t_impacts01[first_hit_dof]
    print(f"Segment 1 ends at t = {t_seg01:.4f}  (DOF {first_hit_dof+1} impacts first)")

    # Predict at impact time
    x1_pred01, xt1_pred01, _ = model01.predict(np.array([[t_seg01]]))

    # Propagate ICs to segment 2
    x0_02, xt0_02, y0_02, yt0_02 = propagate_ics(
        model01,
        t_seg01,
        x1_pred01, xt1_pred01,
        y0, yt0,
        first_hit_dof,
    ...
    print("\nSegment 2 initial conditions ready.")
    print("  x0_02  =", x0_02)
    print("  xt0_02 =", xt0_02)
    print("  y0_02  =", y0_02)
    print("  yt0_02 =", yt0_02)
