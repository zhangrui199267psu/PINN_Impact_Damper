...
class PIPNNs:
    ...
    def __init__(self, lb, ub, t0, t, x0_total, xt0_total, y03, yt03, y04, yt04, M, K, D, n_dof, 
                 phi, phi1, phi2, layers, hyp_ini_weight_loss, hyp_ini_para, 
                 optimizer_LB1, optimizer_LB2, optimizer_LB3):
        ...
        self.lambda_2 = tf.Variable(hyp_ini_para[1], dtype=tf.float32)
        ...
        if optimizer_LB2:
            self.optimizer2 = tf.contrib.opt.ScipyOptimizerInterface(self.loss_f1,
                                                                ...
                                                                options={'maxiter': 50000,
                                                                         ...
                                                                         'ftol': 1.0 * np.finfo(float).eps})
        if optimizer_LB3:
            self.optimizer3 = tf.contrib.opt.ScipyOptimizerInterface(self.loss_f2,
                                                                var_list=[self.lambda_2],
                                                                method='L-BFGS-B',
                                                                options={'maxiter': 50000,
                                                                         'maxfun': 50000,
                                                                         'maxcor': 50,
        ...
        self.train_op_Adam2 = self.optimizer_Adam2.minimize(self.loss_f1, var_list=[self.lambda_1])
        
        self.optimizer_Adam3 = tf.train.AdamOptimizer()
        self.train_op_Adam3 = self.optimizer_Adam3.minimize(self.loss_f2, var_list=[self.lambda_2])
    ...
    def callback2(self, loss_f1, lambda_1):

        # Store loss
        self.loss_f1_log.append(loss_f1)

        # Store lambda
        self.lambda_1_log.append(lambda_1)

        print('Loss_f: %.5e, l1: %.5f'
              % (loss_f1, lambda_1))
    ...
    def train2(self, nIter, optimizer_LB2):
        ...
        for it in range(nIter):
            ...
            if it % 10 == 0:
                ...
                loss_f1_value = self.sess.run(self.loss_f1, tf_dict)
                ...
                print('It: %d,  Loss_f: %.3e, Lambda_1: %.3f, Time: %.2f' %
                    (it, loss_f1_value, lambda_1_value,  elapsed))
                start_time = time.time()

        if optimizer_LB2:
            self.optimizer2.minimize(self.sess,
                                feed_dict=tf_dict,
                                fetches=[self.loss_f1,
                                         self.lambda_1],
                                loss_callback=self.callback2)
            
    def callback3(self, loss_f2, lambda_2):

        # Store loss
        self.loss_f2_log.append(loss_f2)

        # Store lambda
        self.lambda_2_log.append(lambda_2)

        print('Loss_f: %.5e, l2: %.5f'
              % (loss_f2, lambda_2))

    def train3(self, nIter, optimizer_LB3):
        ...
        for it in range(nIter):
            ...
            if it % 10 == 0:
                ...
                loss_f2_value = self.sess.run(self.loss_f2, tf_dict)

                # Store loss
                self.loss_f2_log.append(loss_f2_value)
                ...
                print('It: %d,  Loss_f: %.3e, Lambda_2: %.3f, Time: %.2f' %
                    (it, loss_f2_value, lambda_2_value, elapsed))
                start_time = time.time()

        if optimizer_LB3:
            self.optimizer3.minimize(self.sess,
                                feed_dict=tf_dict,
                                fetches=[self.loss_f2,
                                         self.lambda_2],
                                loss_callback=self.callback3)
